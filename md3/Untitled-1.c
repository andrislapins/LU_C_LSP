...
typedef struct entry_t {
    ...
    char* date;
    struct entry_t* next;
    int count; // Count of entries of same properties. // Link them together.
    int visited; // Check if vistited.// if yes then do not print again.
    struct entry_t* next_similar;
} entry_t;
...
entry_t* ht_pair(const char* key, const char* value)
{
    ...
    struct tm lt; // Result of localtime_r ...
    time_t t = file_info.st_mtime; // Store file date.
    char timbuf[80];
...
void ht_dump(ht_t* hashtable)
{
    for (int i = 0; i < TABLE_SIZE; ++i) {
        ...
        // sorting here OR in a function
        entry_t* head1 = entry;
        entry_t* head2 = entry;
        while (head1 != NULL) {
            while (head2->next != NULL) {
                if (head1->size == head2->size) {
                    entry_t* trav = malloc(sizeof(entry_t) * 1);
                    trav = head1->next_similar;

                    while(trav != NULL) {
                        trav = trav->next_similar;
                    }

                    trav = head2;
                    trav->next_similar = NULL;
                }
        ...
        for(;;) {
            // if (entry->visited == 1) {
                printf("%s(%d-%d-%ld)\n", entry->value, entry->count, entry->visited, entry->size);
            // }


            if (entry->next_similar == NULL) {
                break;
            }

            entry = entry->next_similar;
        }
